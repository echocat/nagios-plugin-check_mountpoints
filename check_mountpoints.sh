...
if [ ${AUTO} -eq 1 ]; then
        MPS=`${GREP} -v '^#' ${FSTAB} | awk '{if ($'${FSF}'=="ext4" || $'${FSF}'=="nfs" || $'${FSF}'=="nfs4" || $'${FSF}'=="davfs" || $'${FSF}'=="cifs" || $'${FSF}'=="fuse" || $'${FSF}'=="glusterfs" || $'${FSF}'=="ocfs2" || $'${FSF}'=="lustre"){print $'${MF}' }}' | tr '\n' ' '`
fi
...
for MP in ${MPS} ; do
        ...
        if [ ! -f /proc/vz/veinfo -a ${AUTO} -ne 1 -a ${IGNOREFSTAB} -ne 1 ]; then
                awk '{if ($'${FSF}'=="ext4" || $'${FSF}'=="nfs" || $'${FSF}'=="nfs4" || $'${FSF}'=="davfs" || $'${FSF}'=="cifs" || $'${FSF}'=="fuse" || $'${FSF}'=="glusterfs" || $'${FSF}'=="ocfs2" || $'${FSF}'=="lustre"){print $'${MF}'}}' ${FSTAB} | ${GREP} -q ${MP} &>/dev/null
                if [ $? -ne 0 ]; then
        ...
        ## check kernel mounts
        ${GREP} "${MP}" ${MTAB} | ${GREP} -q -E "(ext4|nfs|nfs4|davfs|cifs|fuse|simfs|glusterfs|ocfs2|lustre)" &>/dev/null
        if [ $? -ne 0 ]; then
